...
class GameState
  include Contract
  attr_reader :rows, :columns, :players, :on_change, :board, :on_turn_change, :type
  attr_accessor :player_turn, :last_played
  ...
  method_contract(
      #preconditions
      [lambda { |obj, players, type, rows, columns| rows.respond_to?(:to_i) },
       lambda { |obj, players, type, rows, columns| rows.to_i > 0 },
       lambda { |obj, players, type, rows, columns| columns.respond_to?(:to_i) },
       lambda { |obj, players, type, rows, columns| columns.to_i > 0 },
       lambda { |obj, players, type, rows, columns| players.respond_to?(:to_i) },
       lambda { |obj, players, type, rows, columns| players.to_i > 1 },
       lambda { |obj, players, type, rows, columns| type.respond_to?(:to_s)}],
      #postconditions
      [])

  def initialize(players, type, rows, columns)
    @rows = rows.to_i
